...
public class MateriaDAO {
    public void insertarMateria(int alumnoId, String materia, int horas) {
        try (Connection conn = Connect.getConnection()) {
            String sql = "INSERT INTO MateriasEsp (alumno_id, nombre, horas) VALUES (?, ?, ?)";
    ...
    public void eliminarMateria(int alumnoId, String materia) {
        try (Connection conn = Connect.getConnection()) {
            String sql = "DELETE FROM MateriasEsp WHERE alumno_id = ? AND nombre = ?";
            PreparedStatement stmt = conn.prepareStatement(sql);
            stmt.setInt(1, alumnoId);
            stmt.setString(2, materia);
            stmt.executeUpdate();
    ...
    public List<Materia> obtenerMateriasPorAlumno(int alumnoId) {
        ...
        String sql = "SELECT A.nombre AS nombre_alumno, M.nombre AS nombre_materia, ME.nombre AS nombre_materiaesp " +
                     "FROM Alumno A " +
                     "JOIN Materia M ON M.id_curso = A.id_curso " +
                     "JOIN MateriaEsp ME ON ME.id_alumno = A.id " +
                     "WHERE A.id = ?";
        ...
        try (Connection conn = Connect.getConnection();
            ...
            while (rs.next()) {
                ...
                resultados.add(new Materia(rs.getInt("curso_id"), rs.getString("nombre"), rs.getString("materia"), rs.getInt("horas")));
            }
    ...
    public void insertarMateriaPorCurso(int cursoId, String materia, int horas) {
        try (Connection conn = Connect.getConnection()) {
            ...
            String sql = "INSERT INTO Materias (curso_id, nombre, horas) " +
    ...
    public void eliminarMateriaPorCurso(int cursoId, String materia) {
        try (Connection conn = Connect.getConnection()) {
            ...
            String sql = "DELETE FROM Materias WHERE nombre = ? AND _id IN " +
                         "(SELECT id_curso FROM Alumnos WHERE curso_id = ?)";
            PreparedStatement stmt = conn.prepareStatement(sql);
            stmt.setString(1, materia);
    ...
    public List<Materia> obtenerMateriasPorCurso(int cursoId) {
        ...
        try (Connection conn = Connect.getConnection()) {
            ...
            String sql = "SELECT M.curso_id, M.nombre, M.horas FROM Materias M " +
